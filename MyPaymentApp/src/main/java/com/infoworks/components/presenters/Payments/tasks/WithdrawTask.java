...
public class WithdrawTask extends ExecutableTask<Message, Response> {
    ...
    public WithdrawTask(String accountName
        ...
        try {
            if (isExist) {
                ...
                if (response.getStatus() == 200) {
                    ...
                    throw new Exception(accountTitle + " : " + amount + " (" + currency + ") Withdraw Failed!");
                }
            } else {
    ...
    public Response abort(Message message) throws RuntimeException {
        if (shouldReverseIfExecuted) {
            ...
            Response response = getRepository().accountExist(accountPrefix, accountName);
            boolean isExist = VAccountResponseParser.isExist(response);
            //Reverse Withdraw or Make Deposit:
            if (isExist) {
                ...
                if (response.getStatus() == 200) {
                    ...
                    return response.setStatus(500)
                            .setError(accountTitle + " : " + amount + " (" + currency + ") Deposit Failed!");
                }
            } else {

...
import com.infoworks.lab.services.vaccount.*;
...
public class AccountControllerV2 {
    ...
    @PostMapping("/make/transaction/on/handover")
    public ResponseEntity<Response> makeTransactionsOnHandOver(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String token
            ,@Valid @RequestBody MakeTransactionWithCharge transaction) {
        //
        Response response = new Response()
                .setMessage("Successfully Enqueued for Processing.")
                .setStatus(HttpStatus.OK.value());
        ...
        //Now Make The HandOver-Transaction-Flow
        TaskStack stack = TaskStack.createSync(true);
        stack.push(new MakeTransactionOnHandoverTask(ledgerBook, transaction));
        stack.commit(false, (message, state) -> {
            if (state == TaskStack.State.Finished){
                response.setMessage("Transaction Has Successfully Executed!");
            }else{
    ...
    @PostMapping("/make/transaction/on/completion")
    public ResponseEntity<Response> makeTransactionsOnCompletion(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String token
            ,@Valid @RequestBody MakeTransactionWithCharge transaction) {
        //
        Response response = new Response()
        ...
        //Now Make The HandOver-Transaction-Flow
        TaskStack stack = TaskStack.createSync(true);
        stack.push(new MakeTransactionOnCompletionTask(ledgerBook, transaction));
        stack.commit(false, (message, state) -> {
            if (state == TaskStack.State.Finished){
                response.setMessage("Transaction Has Successfully Executed!");
            }else{
